...
class ACPAdoptController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($mysidia->input->post("submit")) {
            ...
            $type_exist = $mysidia->db->select("adoptables", ["type"], "type = '{$name}'")->fetchColumn();
            if ($type_exist) {
            ...
            $mysidia->db->insert("adoptables", ["id" => null, "type" => $mysidia->input->post("type"), "class" => $mysidia->input->post("class"), "description" => $mysidia->input->post("description"), "eggimage" => $eggimage, "whenisavail" => $mysidia->input->post("cba"),
                                                                        "alternates" => $mysidia->input->post("alternates"), "altoutlevel" => $mysidia->input->post("altoutlevel"), "shop" => $mysidia->input->post("shop"), "cost" => $mysidia->input->post("cost")]);

            // insert into table adoptables_conditions
            $mysidia->db->insert("adoptables_conditions", ["id" => null, "type" => $mysidia->input->post("type"), "whenisavail" => $mysidia->input->post("cba"), "freqcond" => $mysidia->input->post("freqcond"), "number" => $mysidia->input->post("number"), "datecond" => $mysidia->input->post("datecond"),
            ...
            $mysidia->db->insert("levels", ["lvid" => null, "adoptiename" => $mysidia->input->post("type"), "thisislevel" => 0, "requiredclicks" => 0, "primaryimage" => $eggimage, "rewarduser" => null, "promocode" => null]);
        ...
        if (!$mysidia->input->post("choose")) {
            ...
            if ($mysidia->session->fetch("acpAdopt") != "edit") {
                ...
                switch ($mysidia->input->post("deltype")) {
                    case "soft":
                        $mysidia->db->update("owned_adoptables", ["currentlevel" => 1], "type='{$mysidia->input->post("type")}' AND currentlevel='0'");
                        $mysidia->db->delete("levels", "adoptiename='{$mysidia->input->post("type")}' and thisislevel=0");
                        $mysidia->db->update("adoptables", ["whenisavail" => "promo"], "type='{$mysidia->input->post("type")}'");
                        $mysidia->db->update("adoptables_conditions", ["whenisavail" => "promo"], "type='{$mysidia->input->post("type")}'");
                        break;
                    case "hard":
                ...
                if ($mysidia->input->post("select") != "" and $mysidia->input->post("select") != "none") {
                    $mysidia->db->update("adoptables", ["eggimage" => $mysidia->input->post("select")], "type='{$mysidia->input->post("type")}'");
                }
                if ($mysidia->input->post("resetconds") == "yes") {
                    $mysidia->db->update("adoptables", ["whenisavail" => 'always'], "type='{$mysidia->input->post("type")}'");
                    $mysidia->db->update("adoptables_conditions", ["whenisavail" => 'always'], "type='{$mysidia->input->post("type")}'");
                }
                if ($mysidia->input->post("resetdate") == "yes" and $mysidia->input->post("date")) {
                    $mysidia->db->update("adoptables", ["whenisavail" => "conditions"], "type='{$mysidia->input->post("type")}'");
                    $mysidia->db->update("adoptables_conditions", ["whenisavail" => "conditions", "datecond" => "enabled", "date" => $mysidia->input->post("date")], "type='{$mysidia->input->post("type")}'");
                }
            }
        } else {
            ...
            $this->setField("availtext", new MysString($availtext));
        }
    }
