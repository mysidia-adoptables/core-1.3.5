...
class ACPAdsController extends AppController
{
    ...
    public function edit()
    {
        ...
        if (!$mysidia->input->get("aid")) {
            ...
            $mysidia->db->update("ads", ["adname" => $mysidia->input->post("adname"), "text" => $mysidia->input->post("description"), "page" => $mysidia->input->post("adpage"), "impressions" => $mysidia->input->post("impressions")], "id='{$mysidia->input->get("aid")}'");
            if ($mysidia->input->post("aimp") >= $mysidia->input->post("impressions") and $mysidia->input->post("impressions") != 0) {
                $mysidia->db->update("ads", ["status" => 'inactive'], "id='{$mysidia->input->get("aid")}'");
            } else {
                $mysidia->db->update("ads", ["status" => 'active'], "id='{$mysidia->input->get("aid")}'");
            }
            return;
        } else {
            $ad = $mysidia->db->select("ads", [], "id = '{$mysidia->input->get("aid")}' LIMIT 1")->fetchObject();
            if (!is_object($ad)) {
                throw new InvalidIDException("nonexist");
            }
            $this->setField("ad", new DataMysObject($ad));
        }
    }
