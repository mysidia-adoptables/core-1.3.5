...
class ACPBreedingController extends AppController
{
    ...
    public function index()
    {
        ...
        $pagination = new Pagination($total, $mysidia->settings->pagination, "admincp/breeding");
    ...
    public function add()
    {
        ...
        if ($mysidia->input->post("submit")) {
            ...
            $mysidia->db->insert("breeding", ["bid" => null, "offspring" => $mysidia->input->post("offspring"), "parent" => $mysidia->input->post("parent"), "mother" => $mysidia->input->post("mother"), "father" => $mysidia->input->post("father"),
    ...
    public function edit()
    {
        ...
        if ($mysidia->input->post("submit")) {
            $this->dataValidate();
            $availability = ($mysidia->input->post("available") == "yes") ? "yes" : "no";
            $mysidia->db->update("breeding", ["offspring" => $mysidia->input->post("offspring"), "parent" => $mysidia->input->post("parent"), "mother" => $mysidia->input->post("mother"), "father" => $mysidia->input->post("father"), "probability" => $mysidia->input->post("probability"),
        ...
        if ($mysidia->input->post("submit")) {
            ...
            foreach ($settings as $name) {
                if ($mysidia->input->post($name) != ($breedingSettings->$name)) {
                    $mysidia->db->update("breeding_settings", ["value" => $mysidia->input->post($name)], "name='{$name}'");
                }
            }
            return;
        }
