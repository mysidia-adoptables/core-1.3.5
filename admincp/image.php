...
class ACPImageController extends AppController
{
    ...
    public function upload()
    {
        ...
        if ($mysidia->input->post("submit")) {
            ...
            $ffn = preg_replace("/[^a-zA-Z0-9\\040.]/", "", (string) $mysidia->input->post("ffn"));
            $ffn = secure($ffn);
            if (empty($ffn)) {
    ...
    {
        $mysidia = Registry::get("mysidia");
        $stmt = $mysidia->db->select("filesmap", ["id", "wwwpath"]);
    ...
    public function delete()
    {
        ...
        if (!$mysidia->input->post("iid")) {
            ...
            $file = $mysidia->db->select("filesmap", [], "id='{$mysidia->input->post("iid")}'")->fetchObject();
            if (!is_object($file)) {
