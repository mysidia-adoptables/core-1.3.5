...
class ACPItemController extends AppController
{
    ...
    public function index()
    {
        ...
        $pagination = new Pagination($total, $mysidia->settings->pagination, "admincp/item");
    ...
    public function add()
    {
        ...
        if ($mysidia->input->post("submit")) {
            ...
            $mysidia->db->insert("items", ["category" => $mysidia->input->post("category"), "itemname" => $mysidia->input->post("itemname"), "description" => $mysidia->input->post("description"), "imageurl" => $imageurl, "function" => $mysidia->input->post("function"), "target" => $mysidia->input->post("target"), "value" => $value,
    ...
    public function edit()
    {
        ...
        if (!$mysidia->input->get("id")) {
            ...
            $this->dataValidate();
            $imageurl = ($mysidia->input->post("existingimageurl") == "none") ? $mysidia->input->post("imageurl") : $mysidia->input->post("existingimageurl");
            $mysidia->db->update("items", ["category" => $mysidia->input->post("category"), "itemname" => $mysidia->input->post("itemname"), "imageurl" => $imageurl, "function" => $mysidia->input->post("function"), "target" => $mysidia->input->post("target"), "value" => $mysidia->input->post("value"),
            ...
            $item = $mysidia->db->select("items", [], "id='{$mysidia->input->get("id")}'")->fetchObject();
            if (!is_object($item)) {
                throw new InvalidIDException("nonexist");
            }
            $this->setField("item", new DataMysObject($item));
        }
    }
    ...
    private function dataValidate()
    {
        ...
        if ($this->action == "add") {
            $item = $mysidia->db->select("items", [], "itemname = '{$mysidia->input->post("itemname")}'")->fetchObject();
            if (is_object($item)) {
    ...
    private function update()
    {
        ...
        if ($mysidia->input->post("itemname") != $mysidia->input->post("originame")) {
            ...
            $num = $mysidia->db->select("inventory", [], "itemname='{$mysidia->input->post("originame")}'")->rowCount();
            while ($num > 0) {
                $mysidia->db->update("inventory", ["itemname" => $mysidia->input->post("itemname")], "itemname='{$mysidia->input->post("originame")}'");
                $num--;
            }
        }
        if ($mysidia->input->post("category") != $mysidia->input->post("origincat")) {
            // Update itemnames in user inventory
            $num = $mysidia->db->select("inventory", [], "category='{$mysidia->input->post("origincat")}'")->rowCount();
            while ($num > 0) {
                $mysidia->db->update("inventory", ["category" => $mysidia->input->post("category")], "itemname='{$mysidia->input->post("origincat")}'");
                $num--;
            }
        }
