...
class ACPThemeController extends AppController
{
    ...
    public function index()
    {
        ...
        $pagination = new Pagination($total, $mysidia->settings->pagination, "admincp/theme");
    ...
    public function edit()
    {
        ...
        if (!$mysidia->input->get("tid")) {
            ...
            $mysidia->db->update("themes", ["themename" => $mysidia->input->post("theme"), "themefolder" => $mysidia->input->post("folder")], "id='{$mysidia->input->get("tid")}'");
            $this->updateTheme();
            return;
        } else {
            $theme = $mysidia->db->select("themes", [], "id='{$mysidia->input->get("tid")}'")->fetchObject();
            if (!is_object($theme)) {
            ...
            $this->setField("theme", new DataMysObject($theme));
        }
    }
