...
class AdoptConditions extends Model
{
    ...
    protected function checkFreqCondition()
    {
        if ($this->freqcond == "enabled") {
            $mysidia = Registry::get("mysidia");
            $freq = $mysidia->db->select("owned_adoptables", ["aid"], "type='{$this->adopt->getType()}'")->rowCount();
            if ($freq >= $this->number) {
    ...
    protected function checkNumberCondition()
    {
        if ($this->moreless == "enabled") {
            $mysidia = Registry::get("mysidia");
            $num = $mysidia->db->select("owned_adoptables", ["aid"], "owner='{$mysidia->user->username}' and type='{$this->adopt->getType()}'")->rowCount();
        ...
        switch ($this->adopt->getWhenAvailable()) {
            ...
            case "conditions":
                try {
                    ...
                    return true;
                } catch (AdoptConditionsException) {
                    return false;
                }
            default:
                return false;
