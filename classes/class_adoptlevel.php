...
class AdoptLevel extends Model
{
    ...
    public function __construct($adopt, $level)
    {
        ...
        $row = $mysidia->db->select("levels", [], "adoptiename='{$adopt}' and thisislevel='{$level}'")->fetchObject();
        if (!is_object($row)) {
    ...
    {
        $mysidia = Registry::get("mysidia");
        $mysidia->db->update("levels", [$field => $value], "adoptiename='{$this->adoptiename}' and thisislevel='{$this->thisislevel}'");
    }
}
