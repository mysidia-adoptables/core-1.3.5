...
class Adoptshop extends AdoptContainer
{
    ...
    public function __construct($shopname)
    {
        ...
        $row = $mysidia->db->select("shops", [], "shopname ='{$shopname}'")->fetchObject();
        if (!is_object($row)) {
    ...
    public function getshop()
    {
        ...
        if (empty($this->shopname)) {
            ...
            $stmt = $mysidia->db->select("shops", [], "shopname ='{$this->shopname}'");
    ...
    public function getadopttypes()
    {
        if (!$this->adopts) {
            ...
            $stmt = $mysidia->db->join("adoptables_conditions", "adoptables_conditions.id = adoptables.id")
                            ->select("adoptables", [], constant("PREFIX")."adoptables.shop ='{$this->shopname}'");
            $adopts = [];
    ...
    public function purchase($adopt)
    {
        ...
        if ($adopt->owner != $mysidia->user->username) {
            ...
            if ($moneyleft >= 0) {
                ...
                $mysidia->db->update("users", ["money" => $moneyleft], "username = '{$adopt->owner}'");
                $status = true;
            } else {
    ...
    {
        $mysidia = Registry::get("mysidia");
        $mysidia->db->update("shops", [$field => $value], "sid='{$this->sid}' and shoptype = 'adoptshop'");
    }
}
?> 