...
class Footer extends Widget
{
    ...
    */
    ...
    protected function setAds()
    {
        ...
        $text = "";
        $ads = $mysidia->db->select("ads", [], "page = '{$page}' and status = 'active' ORDER BY RAND() LIMIT 1")->fetchObject();
        if (is_object($ads)) {
            ...
            $mysidia->db->update("ads", ["actualimpressions" => $actualimpressions], "id='{$aid}'");
            if ($actualimpressions >= $impressions and $impressions != 0) {
                $mysidia->db->update("ads", ["status" => "inactive"], "id='{$aid}'");
            }
        }
    ...
    protected function setCredits()
    {
        ...
        $this->credits->add(new Comment("Copyright © 2011-2020 Mysidia RPG, Inc. All rights reserved."));
