...
class Member extends User
{
    ...
    public function __construct($userinfo)
    {
        ...
        $row = $mysidia->db->select("users", [], $whereclause)->fetchObject();
    ...
    public function login($username = ""): never
    {
        throw new AlreadyLoggedinException($mysidia->lang->global_login);
    }
    ...
    {
        $mysidia = Registry::get("mysidia");
        $online = $mysidia->db->select("online", [], "username = '{$this->username}'")->fetchObject();
        if (is_object($online)) {
    ...
    {
        $mysidia = Registry::get("mysidia");
        $profile = $mysidia->db->select("users_profile", ["uid", "avatar"], "username = '{$this->username}'")->fetchObject();
    ...
    public function changecash($amount)
    {
        ...
        $this->money += $amount;
        if ($this->money >= 0) {
            $mysidia->db->update("users", ["money" => $this->money], "username = '{$this->username}'");
            return true;
        } else {
    ...
    public function getVotes($time = "today")
    {
        ...
        $votes = $mysidia->db->select("vote_voters", ["void"], "username = '{$this->username}' and date = '{$date->format('Y-m-d')}'")->rowCount();
        return $votes;
    }
    ...
    {
        $mysidia = Registry::get("mysidia");
        $option = $mysidia->db->select("users_options", ["theme"], "username = '{$this->username}'")->fetchObject();
        return $option->theme;
    }
    ...
    {
        $mysidia = Registry::get("mysidia");
        $totals = $mysidia->db->select("owned_adoptables", [], "owner = '{$this->username}'")->rowCount();
        return $totals;
    }
    ...
    public function getallpms($folder = "inbox")
    {
        ...
        $totalRows = $mysidia->db->select($table, [], $whereclause)->rowCount();
        return $totalRows;
    }
    ...
    {
        $mysidia = Registry::get("mysidia");
        $stmt = $mysidia->db->select("folders_messages", [], "fromuser='{$this->username}' AND folder='{$folder}' ORDER BY mid DESC LIMIT {$pagination->getLimit()},{$pagination->getRowsperPage()}");
        if ($stmt->rowCount() == 0) {
            throw new MessageNotfoundException();
        } else {
            $fields = new LinkedHashMap();
            $fields->put(new MysString("messagetitle"), null);
            $fields->put(new MysString("fromuser"), new MysString("getProfile"));
            $fields->put(new MysString("touser"), new MysString("getProfile"));
            $fields->put(new MysString("datesent"), null);
            $fields->put(new MysString("mid::read"), new MysString(($folder == "outbox") ? "getOutboxReadLink" : "getDraftReadLink"));
            $fields->put(new MysString("mid::delete"), new MysString(($folder == "outbox") ? "getOutboxDeleteLink" : "getDraftDeleteLink"));
    ...
    public function getcontacts()
    {
        ...
        if (empty($this->contacts)) {
            $this->contacts = $mysidia->db->select("users_contacts", [], "uid = '{$this->uid}'")->fetchObject();
        }
    ...
    {
        $sites = ["website", "facebook", "twitter"];
        $ims = ["msn", "aim", "yim", "skype"];
    ...
    public function getoptions()
    {
        ...
        if (empty($this->options)) {
            $this->options = $mysidia->db->select("users_options", [], "username = '{$this->username}'")->fetchObject();
        }
        return $this->options;
    }
    ...
    public function getstatus()
    {
        ...
        if (empty($this->status)) {
            $this->status = $mysidia->db->select("users_status", [], "username = '{$this->username}'")->fetchObject();
        }
        return $this->status;
    }
}
