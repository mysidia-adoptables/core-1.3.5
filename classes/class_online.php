...
class Online extends UserContainer
{
    ...
    public function getonlinemembers()
    {
        ...
        if (empty($this->members)) {
            $data = $mysidia->db->select("online", [], "username != 'Visitor'")->fetchAll();
    ...
    public function update()
    {
        ...
        if (!$userexist) {
            $mysidia->db->insert("online", ["username" => $username, "ip" => $ip, "session" => $session, "time" => $currenttime]) or die('Cannot insert user data.');
        } else {
