...
class PrivateMessage extends Model implements Message
{
    ...
    public function __construct($mid = 0, $folder = "inbox", $notifier = false)
    {
        ...
        if ($mid == 0) {
            ...
            switch ($folder) {
                case "inbox":
                    $row = $mysidia->db->select("messages", [], "id ='{$mid}'")->fetchObject();
                    if (!is_object($row)) {
                    ...
                    break;
                default:
                    $row = $mysidia->db->select("folders_messages", [], "mid ='{$mid}'")->fetchObject();
                    if (!is_object($row)) {
                        throw new MessageNotfoundException();
                    }
                    $this->mid = $row->mid;
    ...
    {
        $mysidia = Registry::get("mysidia");
        $mysidia->db->update("messages", ["status" => 'read'], "id='{$this->mid}'");
    }
    ...
    public function post($user = "")
    {
        ...
        $mysidia->db->insert("messages", ["id" => null, "fromuser" => $this->fromuser, "touser" => $this->touser, "status" => "unread", "datesent" => $date->format("Y-m-d"), "messagetitle" => $this->messagetitle, "messagetext" => $this->messagetext]);

        if ($mysidia->input->post("outbox") == "yes") {
            $mysidia->db->insert("folders_messages", ["mid" => null, "fromuser" => $mysidia->user->username, "touser" => $mysidia->input->post("recipient"), "folder" => "outbox", "datesent" => $date->format("Y-m-d"), "messagetitle" => $mysidia->input->post("mtitle"), "messagetext" => $this->format($mysidia->input->post("mtext"))]);
        }
    ...
    public function postDraft($user = "")
    {
        ...
        $mysidia->db->insert("folders_messages", ["mid" => null, "fromuser" => $mysidia->user->username, "touser" => $mysidia->input->post("recipient"), "folder" => $this->folder, "datesent" => $date->format("Y-m-d"), "messagetitle" => $mysidia->input->post("mtitle"), "messagetext" => $this->format($mysidia->input->post("mtext"))]);
        return true;
    }
    ...
    public function editDraft($user = "")
    {
        ...
        $mysidia->db->update("folders_messages", ["fromuser" => $this->fromuser, "touser" => $this->touser, "folder" => "draft", "datesent" => $date->format("Y-m-d"), "messagetitle" => $this->messagetitle, "messagetext" => $this->messagetext], "fromuser='{$mysidia->user->username}' and mid='{$mysidia->input->post("id")}'");
        return true;
    }
    ...
    public function report()
    {
        ...
        $mysidia->db->insert("messages", ["id" => null, "fromuser" => $mysidia->user->username, "touser" => $mysidia->input->post("recipient"), "status" => "unread", "datesent" => $date->format("Y-m-d"), "messagetitle" => $this->messagetitle, "messagetext" => $this->messagetext]);
        return true;
    }
    ...
    {
        $mysidia = Registry::get("mysidia");
        $mysidia->db->update("messages", [$field => $value], "id='{$this->mid}'");
    }
}
