...
class Promocode
{
    ...
    public function __construct($code = "")
    {
        ...
        $stmt = $mysidia->db->select("promocodes", [], "code ='{$code}'");
    ...
    private function checkdate()
    {
        ...
        if (empty($this->fromdate) and empty($this->todate)) {
            ...
            $timerange = [strtotime((string) $this->fromdate), strtotime((string) $this->todate)];
    ...
    public function execute()
    {
        ...
        switch ($this->type) {
            case "Adopt":
                ...
                $genders = ['f', 'm'];
                $rand = random_int(0, 1);
                $mysidia->db->insert("owned_adoptables", ["aid" => null, "type" => $this->reward, "name" => $this->reward, "owner" => $this->user, "currentlevel" => 0, "totalclicks" => 0, "code" => $code,
                                                               "imageurl" => null, "alternate" => 0, "tradestatus" => 'fortrade', "isfrozen" => 'no', "gender" => $genders[$rand], "lastbred" => 0]);
    ...
    private function usepromo()
    {
        ...
        $this->availability--;
        $mysidia->db->update("promocodes", ["availability" => $this->availability], "pid='{$this->pid}'");
        return true;
    }
}
?> 