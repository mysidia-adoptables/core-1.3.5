...
class Sidebar extends Widget
{
    ...
    */
    ...
    protected function setLinks(LinksList $linksList)
    {
        ...
        while ($sideLink = $stmt->fetchObject()) {
            ...
            if ($sideLink->linkurl == "messages") {
                $num = $mysidia->db->select("messages", ["touser"], "touser='{$mysidia->user->username}' and status='unread'")->rowCount();
                if ($num > 0) {
    ...
    protected function setWolBar()
    {
        ...
        $online = $mysidia->db->select("online", [], "username != 'Visitor'")->rowCount();
        $offline = $mysidia->db->select("online", [], "username = 'Visitor'")->rowCount();
