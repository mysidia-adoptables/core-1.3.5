...
class StockAdopt
{
    ...
    public function __construct($adopttype)
    {
        ...
        $row = $mysidia->db->select("adoptables", [], "type ='{$adopttype}'")->fetchObject();
    ...
    protected function getaltstatus($level)
    {
        ...
        if ($run == "yes") {
            $randnum = random_int(1, $row->altchance);
            if ($randnum == 1) {
                $altstatus = "yes";
    ...
    public function append($owner = "")
    {
        ...
        $genders = ['f', 'm'];
        $rand = random_int(0, 1);
        $mysidia->db->insert("owned_adoptables", ["aid" => null, "type" => $this->type, "name" => $this->type, "owner" => $this->owner,
                                                       "currentlevel" => 0, "totalclicks" => 0, "code" => $code, "imageurl" => "", "alternate" => 0,
