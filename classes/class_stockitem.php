...
class StockItem extends Item
{
    ...
    public function __construct($iteminfo, public $quantity = 0)
    {
    ...
    public function getoldquantity($owner = "")
    {
        ...
        $oldquantity = $mysidia->db->select("inventory", ["quantity"], "itemname ='{$this->itemname}' and owner ='{$this->owner}'")->fetchColumn();
        return $oldquantity;
    }
    ...
    public function append($quantity = 1, $owner = "")
    {
        ...
        if ($oldquantity > 0) {
            ...
            $mysidia->db->update("inventory", ["quantity" => $newquantity], "itemname ='{$this->itemname}' and owner='{$this->owner}'");
        } else {
            // the item does not exist yet, insert a new row into user's inventory
            $mysidia->db->insert("inventory", ["iid" => null, "category" => $this->category, "itemname" => $this->itemname, "owner" => $this->owner, "quantity" => $quantity, "status" => 'Available']);
        }
