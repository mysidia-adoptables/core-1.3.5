...
final class Trade extends MysObject
{
    private $validator;

    public function __construct(private readonly TradeOffer $offer, private readonly TradeSetting $settings)
    {
    }
    ...
    public function getValidator()
    {
        ...
        foreach ($validations as $validation) {
            $validationList->add(new MysString($validation));
        }
    ...
    {
        $validations = match ($type) {
            "public" => $validations = new ArrayObject(["public", "offered", "wanted", "adoptOffered", "adoptPublic", "itemOffered", "itemPublic", "cashOffered", "species", "interval", "number", "duration", "status", "usergroup", "item"]),
            "private" => new ArrayObject(["recipient", "offered", "wanted", "adoptOffered", "adoptWanted", "itemOffered", "itemWanted", "cashOffered", "species", "interval", "number", "duration", "status", "usergroup", "item"]),
            "partial" => new ArrayObject(["recipient", "partial", "adoptOffered", "adoptWanted", "itemOffered", "itemWanted", "cashOffered", "species", "interval", "number", "duration", "usergroup", "item"]),
            default => new ArrayObject($default),
        };
        return $validations;
    }
    ...
    public function offer()
    {
        ...
        $mysidia->db->insert("trade", ["tid" => null, "type" => $this->offer->getType(), "sender" => $this->offer->getSender(), "recipient" => $this->offer->getRecipient(),
                                            "adoptoffered" => $adoptOffered, "adoptwanted" => $adoptWanted, "itemoffered" => $itemOffered, "itemwanted" => $itemWanted,
                                            "cashoffered" => $this->offer->getCashOffered(), "message" => stripslashes((string) $this->offer->getMessage()), "status" => $this->offer->getStatus(), "date" => $this->offer->getDate()]);
    ...
    {
        $mysidia = Registry::get("mysidia");
        $mysidia->db->insert("trade_associations", ["taid" => null, "publicid" => $publicID, "privateid" => $privateID]);
    }

    public function syncronize()
    {
        $mysidia = Registry::get("mysidia");
        $publicID = $mysidia->db->select("trade_associations", ["publicid"], "privateid = '{$this->offer->getID()}'")->fetchColumn();
        if ($publicID) {
            $publicOffer = new TradeOffer($publicID);
            $publicOffer->setStatus("complete", "update");
            $privateIDs = $mysidia->db->select("trade_associations", ["privateid"], "publicid = '{$publicID}' AND privateid != '{$this->offer->getID()}'")->fetchAll(PDO::FETCH_COLUMN);
    ...
    public function revise()
    {
        ...
        $mysidia->db->update("trade", ["adoptoffered" => $adoptOffered, "adoptwanted" => $adoptWanted, "itemoffered" => $itemOffered, "itemwanted" => $itemWanted,
                                            "cashoffered" => $this->offer->getCashOffered(), "message" => stripslashes((string) $this->offer->getMessage()), "status" => $this->offer->getStatus(),
    ...
    public function reverse()
    {
        ...
        $mysidia->db->update("trade", ["type" => $this->offer->getType(), "sender" => $this->offer->getSender(), "recipient" => $this->offer->getRecipient(), "adoptoffered" => $adoptOffered, "adoptwanted" => $adoptWanted, "itemoffered" => $itemOffered, "itemwanted" => $itemWanted,
                                            "cashoffered" => $this->offer->getCashOffered(), "message" => stripslashes((string) $this->offer->getMessage()), "status" => $this->offer->getStatus(),
                                            "date" => $this->offer->getDate()], "tid = {$mysidia->input->get("id")}");
        $this->sendTradeMessage(
            $this->offer->getSender(),
