...
class VisitorMessage extends Model implements Message
{
    ...
    public function __construct($vid = 0, $notifier = false)
    {
        ...
        if ($vid == 0) {
            ...
            $row = $mysidia->db->select("visitor_messages", [], "vid ='{$vid}'")->fetchObject();
            if (!is_object($row)) {
    ...
    public function post($user = "")
    {
        ...
        if ($this->vid != 0) {
            ...
            $date = new DateTime();
            $mysidia->db->insert("visitor_messages", ["vid" => null, "fromuser" => $this->fromuser, "touser" => $this->touser, "datesent" => $date->format("Y-m-d H:i:s"), "vmtext" => $mysidia->input->post("vmtext")]);
        }
        return true;
    }
    ...
    {
        $mysidia = Registry::get("mysidia");
        $mysidia->db->update("visitor_messages", ["vmtext" => $mysidia->input->post("vmtext")], "vid='{$this->vid}' and fromuser='{$this->fromuser}'");
    }
