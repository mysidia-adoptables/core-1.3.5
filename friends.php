...
class FriendsController extends AppController
{
    ...
    public function request()
    {
        ...
        if (!$friend->isfriend) {
            if ($friend->sendrequest()) {
                $this->setField("friend", new MysString($friend->username));
            } else {
    ...
    public function option()
    {
        ...
        $optionsMap->put(new MysString("pmoption"), new Integer($options->pmstatus));
        $optionsMap->put(new MysString("vmoption"), new Integer($options->vmstatus));
        $optionsMap->put(new MysString("tradeoption"), new Integer($options->tradestatus));
    ...
    public function edit()
    {
        ...
        switch ($mysidia->input->get("confirm")) {
            case "accept":
                ...
                $this->setField("fromuser", new MysString($friendrequest->fromuser));
                break;
            case "decline":
                $friendrequest = new FriendRequest($mysidia->input->get("id"));
                ...
                break;
            default:
                $stmt = $mysidia->db->select("friend_requests", [], "touser='{$mysidia->user->username}' and status='pending'");
                if ($stmt->rowCount() == 0) {
