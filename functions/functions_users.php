...
function getgroup()
{
    ...
    if ($mysidia->user->isloggedin) {
        $usergroup = $mysidia->db->select("users", ["usergroup"], "username='{$mysidia->user->username}'")->fetchColumn();
        return $usergroup;
    } else {
        return 0;
    }
...
function cancp($usergroup)
{
    ...
    $mysidia = Registry::get("mysidia");
    $cancp = $mysidia->db->select("groups", ["cancp"], "gid = '{$usergroup}'")->fetchColumn();
...
function canadopt($aid, $cond, $promocode, $row)
{
    ...
    if ($row->whenisavail == "conditions") {
        ...
        if ($row->freqcond == "enabled") {
            ...
            $num = $mysidia->db->select("owned_adoptables", ["aid", "type"], "type='{$type}'")->fetchAll();
            if (count($num) > $number) {
                return "no";
            }
        }
        ...
        if ($row->moreless == "enabled") {
            $num = 0;
            $num = $mysidia->db->select("owned_adoptables", ["aid", "type"], "owner='{$loggedinname}' and type='{$type}'")->fetchAll();
...
function isbanned($user)
{
    $mysidia = Registry::get("mysidia");
    $banstatus = 0;
    $usergroup = $mysidia->db->select("users", ["usergroup"], "username='{$user}'")->fetchColumn();
    if ($usergroup == 5) {
        $banstatus = 1;
    }
    return $banstatus;
...
function banuser($user)
{
    ...
    $mysidia = Registry::get("mysidia");
    $mysidia->db->update("users", ["usergroup" => 5], "username = '{$user}'");

    // Then update all user permissions to no
    $mysidia->db->update("users_status", ["canlevel" => 'no', "canvm" => 'no', "canfriend" => 'no', "cantrade" => 'no', "canbreed" => 'no', "canpound" => 'no', "canshop" => 'no'], "username = '{$user}'");
    return true;
}
...
function unbanuser($user)
{
    ...
    $mysidia = Registry::get("mysidia");
    $mysidia->db->update("users", ["usergroup" => 3], "username = '{$user}'");

    // Then update all user permissions to no
    $mysidia->db->update("users_status", ["canlevel" => 'yes', "canvm" => 'yes', "canfriend" => 'yes', "cantrade" => 'yes', "canbreed" => 'yes', "canpound" => 'yes', "canshop" => 'yes'], "username = '{$user}'");
    return true;
}
