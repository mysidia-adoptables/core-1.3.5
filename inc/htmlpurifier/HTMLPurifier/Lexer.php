...
class HTMLPurifier_Lexer
{
    ...
    public static function create($config)
    {
        ...
        if (is_object($lexer)) {
            ...
            $inst = match ($lexer) {
                ...
                default => throw new HTMLPurifier_Exception("Cannot instantiate unrecognized Lexer type " . htmlspecialchars((string) $lexer)),
            };
        }
    ...
    public function parseData($string)
    {
        ...
        $num_amp = substr_count((string) $string, '&') - substr_count((string) $string, '& ') -
            ($string[strlen((string) $string) - 1] === '&' ? 1 : 0);

        if (!$num_amp) {
            return $string;
        } // abort if no entities
        $num_esc_amp = substr_count((string) $string, '&amp;');
