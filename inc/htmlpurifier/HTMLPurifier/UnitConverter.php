...
class HTMLPurifier_UnitConverter
{
    ...
    {
        if ($this->bcmath) {
            return bcadd((string) $s1, (string) $s2, $scale);
        } else {
    ...
    {
        if ($this->bcmath) {
            return bcmul((string) $s1, (string) $s2, $scale);
        } else {
            return $this->scale($s1 * $s2, $scale);
        }
    }
    ...
    {
        if ($this->bcmath) {
            return bcdiv((string) $s1, (string) $s2, $scale);
        } else {
            return $this->scale($s1 / $s2, $scale);
        }
    }
    ...
    private function round($n, $sigfigs)
    {
        ...
        if ($this->bcmath) {
            if ($rp >= 0) {
                $n = bcadd((string) $n, $neg . '0.' .  str_repeat('0', $rp) . '5', $rp + 1);
                $n = bcdiv($n, '1', $rp);
            } else {
                // This algorithm partially depends on the standardized
                // form of numbers that comes out of bcmath.
                $n = bcadd((string) $n, $neg . '5' . str_repeat('0', $new_log - $sigfigs), 0);
