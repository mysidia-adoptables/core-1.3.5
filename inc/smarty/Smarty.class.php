...
class Smarty extends Smarty_Internal_TemplateBase
{
     ...
     * smarty version
     */
    public const SMARTY_VERSION = 'Smarty-3.1.12';
    ...
    public function setTemplateDir($template_dir)
    {
        ...
        foreach ((array) $template_dir as $k => $v) {
            $this->template_dir[$k] = rtrim((string) $v, '/\\') . DS;
        }
        ...
        if (is_array($template_dir)) {
            foreach ($template_dir as $k => $v) {
                if (is_int($k)) {
                    ...
                    $this->template_dir[] = rtrim((string) $v, '/\\') . DS;
                } else {
    ...
    {
        if ($index !== null) {
            return $this->template_dir[$index] ?? null;
        }
    ...
    public function setConfigDir($config_dir)
    {
        ...
        foreach ((array) $config_dir as $k => $v) {
            $this->config_dir[$k] = rtrim((string) $v, '/\\') . DS;
        }
    ...
    public function addConfigDir($config_dir, $key = null)
    {
        ...
        if (is_array($config_dir)) {
            foreach ($config_dir as $k => $v) {
                if (is_int($k)) {
                    // indexes are not merged but appended
                    $this->config_dir[] = rtrim((string) $v, '/\\') . DS;
                } else {
                    // string indexes are overridden
                    $this->config_dir[$k] = rtrim((string) $v, '/\\') . DS;
                }
    ...
    {
        if ($index !== null) {
            return $this->config_dir[$index] ?? null;
        }
    ...
    public function setPluginsDir($plugins_dir)
    {
        ...
        foreach ((array)$plugins_dir as $k => $v) {
            $this->plugins_dir[$k] = rtrim((string) $v, '/\\') . DS;
        }
    ...
    public function addPluginsDir($plugins_dir)
    {
        ...
        if (is_array($plugins_dir)) {
            foreach ($plugins_dir as $k => $v) {
                if (is_int($k)) {
                    // indexes are not merged but appended
                    $this->plugins_dir[] = rtrim((string) $v, '/\\') . DS;
                } else {
                    // string indexes are overridden
                    $this->plugins_dir[$k] = rtrim((string) $v, '/\\') . DS;
                }
            ...
            // append new directory
            $this->plugins_dir[] = rtrim((string) $plugins_dir, '/\\') . DS;
        }
    ...
    {
        if ($type !== null) {
            return $this->autoload_filters[$type] ?? [];
        }
    ...
    public function loadPlugin($plugin_name, $check = true)
    {
        ...
        foreach ($this->getPluginsDir() as $_plugin_dir) {
            ...
            foreach ($names as $file) {
                ...
                if ($this->use_include_path && !preg_match('/^([\/\\\\]|[a-zA-Z]:[\/\\\\])/', (string) $_plugin_dir)) {
...
class SmartyException extends Exception
{
    ...
    {
        $this->message = htmlentities((string) $message);
    }
}
...
{
    $_class = strtolower((string) $class);
    $_classes = [
