...
function smarty_outputfilter_trimwhitespace($source, Smarty_Internal_Template $smarty)
{
    ...
    if (preg_match_all('#<!--\[[^\]]+\]>.*?<!\[[^\]]+\]-->#is', (string) $source, $matches, PREG_OFFSET_CAPTURE | PREG_SET_ORDER)) {
    ...
    $_offset = 0;
    if (preg_match_all('#<(script|pre|textarea)[^>]*>.*?</\\1>#is', (string) $source, $matches, PREG_OFFSET_CAPTURE | PREG_SET_ORDER)) {
        foreach ($matches as $match) {
            $store[] = $match[0][0];
            $_length = strlen($match[0][0]);
            $replace = '@!@SMARTY:' . $_store . ':SMARTY@!@';
