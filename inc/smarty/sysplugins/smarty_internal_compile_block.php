...
class Smarty_Internal_Compile_Block extends Smarty_Internal_CompileBase
{
    ...
    {
        $_rdl = preg_quote((string) $template->smarty->right_delimiter);
        $_ldl = preg_quote((string) $template->smarty->left_delimiter);
                ...
                if (0 != preg_match_all("!({$_ldl}{$al}block\s+)(name=)?(\w+|'.*'|\".*\")([\s\S]*?)(hide)?(\s*{$_rdl})([\s\S]*?)({$_ldl}{$al}\\\$smarty\.block\.child\s*{$_rdl})([\s\S]*?{$_ldl}{$al}/block\s*{$_rdl})!", $block_content, $_match2)) {
                    foreach ($_match2[3] as $key => $name) {
                        ...
                        if ($_match2[5][$key] != 'hide' || isset($template->block_data[$_name2])) {
                            ...
                            $block_content = preg_replace($search, $replace, (string) $block_content);
                        } else {
                            // remove hidden blocks
                            $block_content = preg_replace("!({$_ldl}{$al}block[\s\S]*?{$name}[\s\S]*?{$_rdl}[\s\S]*?{$_ldl}{$al}/block\s*{$_rdl})!", '', (string) $block_content);
                        }
                    }
                }
                // do we have not nested {$smart.block.child}
                if (0 != preg_match("!({$_ldl}{$al}\\\$smarty\.block\.child\s*{$_rdl})!", (string) $block_content, $_match2)) {
                    ...
                    $block_content = preg_replace("!({$_ldl}{$al}\\\$smarty\.block\.child\s*{$_rdl})!", (string) $replacement, (string) $block_content);
                }
                if (isset($template->block_data[$_name])) {
                    if (str_contains((string) $template->block_data[$_name]['source'], '%%%%SMARTY_PARENT%%%%')) {
    ...
    public static function compileChildBlock($compiler, $_name = null)
    {
        ...
        if ($_name == null) {
            ...
            while (--$stack_count >= 0) {
                if ($compiler->_tag_stack[$stack_count][0] == 'block') {
                    $_name = trim((string) $compiler->_tag_stack[$stack_count][1][0]['name'], "'\"");
                    break;
                }
            }
        ...
        if (str_contains($compiler->template->block_data[$_name]['source'], '%%%%SMARTY_PARENT%%%%')) {
...
class Smarty_Internal_Compile_Blockclose extends Smarty_Internal_CompileBase
{
    ...
    public function compile($args, $compiler)
    {
        ...
        $_name = trim((string) $saved_data[0]['name'], "\"'");
