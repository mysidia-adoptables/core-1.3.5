...
class Smarty_Internal_Compile_Extends extends Smarty_Internal_CompileBase
{
    ...
    */
    public function compile($args, $compiler)
    {
        ...
        $this->_rdl = preg_quote((string) $compiler->smarty->right_delimiter);
        $this->_ldl = preg_quote((string) $compiler->smarty->left_delimiter);
        ...
        if (isset($_is_stringy[$_template->source->type])) {
            $template_sha1 = sha1((string) $include_file);
        } else {
            $template_sha1 = sha1((string) $_template->source->filepath);
        }
        if (isset($compiler->template->properties['file_dependency'][$template_sha1])) {
            $compiler->trigger_template_error("illegal recursive call of \"{$include_file}\"", $compiler->lex->line - 1);
        }
        $compiler->template->properties['file_dependency'][$template_sha1] = [$_template->source->filepath, $_template->source->timestamp, $_template->source->type];
        $_content = ($this->mbstring_overload ? mb_substr((string) $compiler->template->source->content, $compiler->lex->counter - 1, 20000000, 'latin1') : substr((string) $compiler->template->source->content, $compiler->lex->counter - 1));
        ...
        while ($_start + 1 < $_result_count) {
            $_end = 0;
            $_level = 1;
            if (($this->mbstring_overload ? mb_substr($_result[0][$_start][0], 0, mb_strlen((string) $compiler->smarty->left_delimiter, 'latin1') + 1, 'latin1') : substr($_result[0][$_start][0], 0, strlen((string) $compiler->smarty->left_delimiter) + 1)) == $compiler->smarty->left_delimiter.'*') {
                $_start++;
                continue;
            }
            while ($_level != 0) {
                $_end++;
                if (($this->mbstring_overload ? mb_substr($_result[0][$_start + $_end][0], 0, mb_strlen((string) $compiler->smarty->left_delimiter, 'latin1') + 1, 'latin1') : substr($_result[0][$_start + $_end][0], 0, strlen((string) $compiler->smarty->left_delimiter) + 1)) == $compiler->smarty->left_delimiter.'*') {
                    continue;
                }
        ...
        if ($_template->source->type == 'extends') {
            ...
            foreach ($_template->source->components as $key => $component) {
                $compiler->template->properties['file_dependency'][$key] = [$component->filepath, $component->timestamp, $component->type];
            }
        }
