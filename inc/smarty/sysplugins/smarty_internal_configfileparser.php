...
class Smarty_Internal_Configfileparser #line 79 "smarty_internal_configfileparser.php"
{
    ...
    public function __construct(private $lex, $compiler)
    {
        // set instance object
        self::instance($this);
    ...
    {
        if (in_array(strtolower((string) $str), ['on','yes','true'])) {
            $res = true;
        } else {
            $res = false;
        }
    ...
    {
        $escaped_string = substr((string) $qstr, 1, strlen((string) $qstr) - 2); //remove outer quotes
    ...
    {
        $inner_str = substr((string) $qstr, 1, strlen((string) $qstr) - 2);
        return stripcslashes($inner_str);
    }

    private static function parse_tripple_double_quoted_string($qstr)
    {
        return stripcslashes((string) $qstr);
    }
