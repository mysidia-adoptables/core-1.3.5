    ...
    public function to_smarty_php()
    {
        ...
        for ($key = 0, $cnt = count($this->subtrees); $key < $cnt; $key++) {
            if ($key + 2 < $cnt) {
                ...
                if (str_ends_with((string) $this->subtrees[$key]->data, '<') && $this->subtrees[$key + 1]->data == '' && str_ends_with((string) $this->subtrees[$key + 2]->data, '?')) {
                    $key = $key + 2;
                    continue;
                }
            }
            if (str_ends_with($code, '<')) {
                ...
                if (str_starts_with((string) $subtree, '?')) {
                    $code = substr($code, 0, strlen($code) - 1) . '<<?php ?>?' . substr((string) $subtree, 1);
                } elseif ($this->parser->asp_tags && str_starts_with((string) $subtree, '%')) {
                    $code = substr($code, 0, strlen($code) - 1) . '<<?php ?>%' . substr((string) $subtree, 1);
                } else {
                    $code .= $subtree;
                }
            ...
            if ($this->parser->asp_tags && str_ends_with($code, '%')) {
                $subtree = $this->subtrees[$key]->to_smarty_php();
                if (str_starts_with((string) $subtree, '>')) {
                    $code = substr($code, 0, strlen($code) - 1) . '%<?php ?>>' . substr((string) $subtree, 1);
                } else {
