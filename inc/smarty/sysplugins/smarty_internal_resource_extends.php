    ...
    public function getContent(Smarty_Template_Source $source)
    {
        ...
        foreach ($_components as $_component) {
            ...
            if ($_component != $_first) {
                $source->template->properties['file_dependency'][$_component->uid] = [$_component->filepath, $_component->timestamp, $_component->type];
            }
            ...
            // extend sources
            if ($_component != $_last) {
                if (preg_match_all("!({$_ldl}{$al}block\s(.+?)\s*{$_rdl})!", (string) $_content, $_open) !=
                preg_match_all("!({$_ldl}{$al}/block\s*{$_rdl})!", (string) $_content, $_close)) {
                    throw new SmartyException("unmatched {block} {/block} pairs in template {$_component->type} '{$_component->name}'");
                }
                preg_match_all("!{$_ldl}{$al}block\s(.+?)\s*{$_rdl}|{$_ldl}{$al}/block\s*{$_rdl}|{$_ldl}\*([\S\s]*?)\*{$_rdl}!", (string) $_content, $_result, PREG_OFFSET_CAPTURE);
                ...
                while ($_start + 1 < $_result_count) {
                    ...
                    $_block_content = str_replace(
                        ...
                        '%%%%SMARTY_PARENT%%%%',
                        ($this->mbstring_overload ? mb_substr((string) $_content, $_result[0][$_start][1] + mb_strlen($_result[0][$_start][0], 'latin1'), $_result[0][$_start + $_end][1] - $_result[0][$_start][1] - + mb_strlen($_result[0][$_start][0], 'latin1'), 'latin1') : substr((string) $_content, $_result[0][$_start][1] + strlen($_result[0][$_start][0]), $_result[0][$_start + $_end][1] - $_result[0][$_start][1] - + strlen($_result[0][$_start][0])))
                    );
