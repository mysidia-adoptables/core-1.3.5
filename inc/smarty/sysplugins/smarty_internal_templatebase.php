...
abstract class Smarty_Internal_TemplateBase extends Smarty_Internal_Data
{
    ...
    public function fetch($template = null, $cache_id = null, $compile_id = null, $parent = null, $display = false, $merge_tpl_vars = true, $no_output_filter = false)
    {
        ...
        if (!$this->smarty->debugging && $this->smarty->debugging_ctrl == 'URL') {
            ...
            if (str_contains((string) $_query_string, (string) $this->smarty->smarty_debug_id)) {
                if (str_contains((string) $_query_string, $this->smarty->smarty_debug_id . '=on')) {
                    // enable debugging for this browser session
                    setcookie('SMARTY_DEBUG', true);
                    $this->smarty->debugging = true;
                } elseif (str_contains((string) $_query_string, $this->smarty->smarty_debug_id . '=off')) {
        ...
        if ($display) {
            if ($this->caching && $this->cache_modified_check) {
                ...
                $_last_modified_date = @substr((string) $_SERVER['HTTP_IF_MODIFIED_SINCE'], 0, strpos((string) $_SERVER['HTTP_IF_MODIFIED_SINCE'], 'GMT') + 3);
    ...
    public function registerObject($object_name, $object_impl, $allowed = [], $smarty_args = true, $block_methods = [])
    {
        ...
        [$object_impl, (array) $allowed, (bool) $smarty_args, (array) $block_methods];
        return $this;
    }
    ...
    public function _get_filter_name($function_name)
    {
        if (is_array($function_name)) {
            ...
            $function_name[0]::class : $function_name[0]);
