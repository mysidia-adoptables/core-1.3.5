...
class Smarty_Internal_Utility
{
    ...
    public static function compileAllTemplates($extention, $force_compile, $time_limit, $max_errors, Smarty $smarty)
    {
        ...
        foreach ($smarty->getTemplateDir() as $_dir) {
            ...
            foreach ($_compile as $_fileinfo) {
                if (str_starts_with(basename((string) $_file->getPathname()), '.') || str_contains((string) $_fileinfo, '.svn')) {
                    continue;
                }
                $_file = $_fileinfo->getFilename();
                if (!substr_compare((string) $_file, (string) $extention, - strlen((string) $extention)) == 0) {
                    continue;
                }
    ...
    public static function compileAllConfig($extention, $force_compile, $time_limit, $max_errors, Smarty $smarty)
    {
        ...
        foreach ($smarty->getConfigDir() as $_dir) {
            ...
            $_compile = new RecursiveIteratorIterator($_compileDirs);
            foreach ($_compile as $_fileinfo) {
                if (str_starts_with(basename((string) $_fileinfo->getPathname()), '.') || str_contains((string) $_fileinfo, '.svn')) {
                    continue;
                }
                $_file = $_fileinfo->getFilename();
                if (!substr_compare((string) $_file, (string) $extention, - strlen((string) $extention)) == 0) {
