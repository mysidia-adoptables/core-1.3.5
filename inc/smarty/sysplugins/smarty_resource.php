...
abstract class Smarty_Resource
{
    ...
    {
        return static::class . '#' . $smarty->joined_template_dir . '#' . $resource_name;
    }
    ...
    public function populateCompiledFilepath(Smarty_Template_Compiled $compiled, Smarty_Internal_Template $_template)
    {
        ...
        if ($_basename === null) {
            $_basename = basename((string) preg_replace('![^\w\/]+!', '_', $compiled->source->name));
        }
    ...
    protected function normalizePath($_path, $ds = true)
    {
        ...
        // resolve parents
        while (true) {
            $_parent = strpos((string) $_path, '/../', $offset);
            if (!$_parent) {
                break;
            ...
            $_pos = strrpos((string) $_path, '/', $_parent - strlen((string) $_path) - 1);
            if ($_pos === false) {
                // don't we all just love windows?
                $_pos = $_parent;
            }
    ...
    protected function buildFilepath(Smarty_Template_Source $source, Smarty_Internal_Template $_template = null)
    {
        ...
        if (!preg_match('/^([\/\\\\]|[a-zA-Z]:[\/\\\\])/', $file)) {
            foreach ($_directories as $_directory) {
                ...
                if ($source->smarty->use_include_path && !preg_match('/^([\/\\\\]|[a-zA-Z]:[\/\\\\])/', (string) $_directory)) {
...
class Smarty_Template_Source
{
     ...
     */
    public function __construct(Smarty_Resource $handler, Smarty $smarty, /**
    ...
    public function __set($property_name, $value)
    {
        $this->$property_name = match ($property_name) {
            'timestamp', 'exists', 'content', 'template' => $value,
            default => throw new SmartyException("invalid source property '$property_name'."),
        };
    }
