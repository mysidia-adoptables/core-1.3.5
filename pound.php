...
class PoundController extends AppController
{
    ...
    public function index()
    {
        $mysidia = Registry::get("mysidia");
        $pounds = new HashMap();
        $stmt = $mysidia->db->select("pounds", ["aid"], "currentowner='SYSTEM'");
    ...
    protected function getSettings()
    {
        ...
        $stmt = $mysidia->db->select("pound_settings", []);
    ...
    protected function getCost(OwnedAdoptable $adopt, $action = "")
    {
        ...
        if ($this->settings->cost->active == "yes") {
            ...
            $cost = match ($this->settings->cost->advanced) {
                "percent" => $adopt->getCost() * (1 + (0.01 * $costs[$action])),
                default => $adopt->getCost() + $this->settings->$costs[$action],
            };
        }
        if ($this->settings->levelbonus->active == "yes") {
            $cost = match ($this->settings->levelbonus->advanced) {
                "increment" => $adopt->getCost() + ($this->settings->levelbonus->value * $adopt->getCurrentLevel()),
                default => $adopt->getCost() * $adopt->getCurrentLevel(),
            };
        }
        return $cost;
    }
