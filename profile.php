...
class ProfileController extends AppController
{
    ...
    public function index()
    {
        ...
        $stmt = $mysidia->db->select("users", ["username", "usergroup"], "1 ORDER BY uid ASC LIMIT {$pagination->getLimit()},{$pagination->getRowsperPage()}");
        $users = new LinkedHashMap();
        while ($user = $stmt->fetchObject()) {
            $users->put(new MysString($user->username), new Integer($user->usergroup));
        }
