    ...
    public function user()
    {
        ...
        if ($mysidia->input->post("submit")) {
            ...
            if ($whereClause == "1") {
                ...
                $stmt = $mysidia->db->select("users", ["uid"], "{$whereClause} ORDER BY uid DESC");
                if ($stmt->rowCount() == 0) {
    ...
    public function adopt()
    {
        ...
        if ($mysidia->input->post("submit")) {
            ...
            if ($whereClause == "1") {
                ...
                $stmt = $mysidia->db->select("owned_adoptables", ["aid"], "{$whereClause} and owner != 'SYSTEM' ORDER BY aid ASC");
                if ($stmt->rowCount() == 0) {
                    throw new SearchException($mysidia->lang->result);
                }
    ...
    public function item()
    {
        ...
        if ($mysidia->input->post("submit")) {
            ...
            if ($whereClause == "1") {
                ...
                $stmt = $mysidia->db->select("items", ["id"], "{$whereClause} ORDER BY id ASC");
                if ($stmt->rowCount() == 0) {
                    throw new SearchException($mysidia->lang->result);
                }
